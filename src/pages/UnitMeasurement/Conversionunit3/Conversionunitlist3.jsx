...
const Conversionunitlist3 = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState("");

  // Static data for packaging unit conversions
  const conversionUnits = [
    {
      id: "PUC001",
      from_packagingUnit_Name: "Box",
      from_value: 1,
      to_packagingUnit_Name: "Packet",
      to_value: 10,
    },
    {
      id: "PUC002",
      from_packagingUnit_Name: "Carton",
      from_value: 1,
      to_packagingUnit_Name: "Box",
      to_value: 5,
    },
    {
      id: "PUC003",
      from_packagingUnit_Name: "Bottle",
      from_value: 1,
      to_packagingUnit_Name: "Case",
      to_value: 12,
    },
    {
      id: "PUC004",
      from_packagingUnit_Name: "Packet",
      from_value: 100,
      to_packagingUnit_Name: "Carton",
      to_value: 1,
    },
  ];

  const AddConversion = () => {
    navigate("/conversionunit3");
  };
  ...
  const handleExcelDownload = () => {
    try {
      ...
      const exportData = filteredData.map((unit, index) => ({
        "#": index + 1,
        "From Packaging Unit": unit.from_packagingUnit_Name,
        "From Value": unit.from_value,
        "To Packaging Unit": unit.to_packagingUnit_Name,
        "To Value": unit.to_value,
      }));

      const ws = XLSX.utils.json_to_sheet(exportData);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "Packaging Conversions");
      XLSX.writeFile(wb, "Packaging_Unit_Conversions.xlsx");
  ...
  const handleEdit = (id) => {
    navigate("/conversionunit3", { state: { id } });
  };
  ...
  const filteredData = conversionUnits.filter(
    (unit) =>
      unit.from_packagingUnit_Name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      unit.to_packagingUnit_Name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Measurement Units" breadcrumbItem="All Packaging Unit Conversions" />
              ...
              >
                <CardBody className="p-4">
                  <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap">
                    ...
                    >
                      Packaging Unit Conversion List
                    </h4>
                  ...
                  <div className="table-container">
                    ...
                    >
                      ...
                      >
                        <tr>
                          <th>Sr.No</th>
                          <th>From Packaging Unit</th>
                          <th>From Value</th>
                          <th>To Packaging Unit</th>
                          <th>To Value</th>
                          <th>Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredData.length === 0 ? (
                          ...
                          filteredData.map((unit, index) => (
                            <tr key={unit.id}>
                              <td>{index + 1}</td>
                              <td>{unit.from_packagingUnit_Name}</td>
                              <td>{unit.from_value}</td>
                              <td>{unit.to_packagingUnit_Name}</td>
                              <td>{unit.to_value}</td>
                              <td>
      ...
      <style jsx>{`
        ...
        .table {
          margin-bottom: 0;
          min-width: 700px; /* Adjusted for packaging conversion columns */
        }
...
export default Conversionunitlist3;
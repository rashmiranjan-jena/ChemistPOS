...
const validationSchema = Yup.object({
  measurementUnit: Yup.string().required("Please select the Measurement Unit"),
  value1: Yup.number()
    .required("Please enter the first value")
    .positive("Value must be positive"),
  packagingUnit: Yup.string().required("Please select the Packaging Unit"),
  value2: Yup.number()
    .required("Please enter the second value")
    .positive("Value must be positive"),
});

const Conversionunit2 = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { id } = location.state || {};
  const isEditMode = !!id;

  // Static data for measurement and packaging units
  ...
  const staticEditData = {
    ...
    from_value: 1,
    to_packagingUnit: "PU001",
    to_value: 10,
  };

  const initialValues = {
    measurementUnit: isEditMode ? staticEditData.from_measurementUnit : "",
    value1: isEditMode ? staticEditData.from_value : "",
    packagingUnit: isEditMode ? staticEditData.to_packagingUnit : "",
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            title="Measurement to Packaging Unit Conversion Registration"
            breadcrumbItem={
              ...
              >
                <CardBody className="p-4">
                  <div className="d-flex justify-content-between align-items-center mb-4">
                    ...
                    >
                      {isEditMode
                        ? "Edit Measurement to Packaging Conversion"
                        : "Add Measurement to Packaging Conversion"}
                    </h4>
                    <Button
                      color="secondary"
                      onClick={() => navigate(-1)}
                        ...
                        <Row className="g-4">
                          ...
                          <Col md="6">
                            <FormGroup>
                              ...
                              <Field
                                ...
                                id="packagingUnit"
                                className={`form-control ${
                                  errors.packagingUnit && touched.packagingUnit
                                    ? "is-invalid"
                                    : ""
                                }`}
                                style={{
                          ...
                          <Col md="12" className="text-end">
                            ...
                            <Button
                              type="button"
                              color="secondary"
                              onClick={() => navigate("/conversionunitlist2")}
                              disabled={isSubmitting}
                              style={{
                                padding: "10px 25px",
                                borderRadius: "8px",
...
export default Conversionunit2;
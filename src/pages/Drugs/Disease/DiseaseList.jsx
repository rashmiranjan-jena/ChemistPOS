...
const DiseaseList = () => {
  ...
  const [tempFilterDiseaseName, setTempFilterDiseaseName] = useState("");
  ...
  useEffect(() => {
    const fetchDiseases = async () => {
      try {
        ...
        Swal.fire({
          title: "Error!",
          text: error?.message ?? "Failed to fetch diseases.",
          icon: "error",
  ...
  const handleEdit = (id) => {
    navigate(`/disease-form`, { state: { id } });
  };

  const handleDelete = async (id) => {
    const firstConfirmation = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to delete this disease.",
      icon: "warning",
    ...
    if (firstConfirmation?.isConfirmed) {
      ...
      if (secondConfirmation?.isConfirmed) {
        try {
          ...
          if (diseases.length === 1 && currentPage > 1) {
          ...
          Swal.fire({
            title: "Deleted!",
            text: "The disease has been successfully deleted.",
            icon: "success",
            confirmButtonText: "OK",
          });
        } catch (error) {
          ...
          Swal.fire({
            title: "Error!",
            text: error?.message ?? "Failed to delete the disease.",
            icon: "error",
            confirmButtonText: "OK",
          });
        }
  ...
  const handleExcelUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await uploadDiseasesExcel(formData);
        Swal.fire({
        ...
        const updatedData = await getDiseases(
          currentPage,
          pageSize,
          filterDiseaseName
        );
        setDiseases(updatedData?.data ?? []);
  ...
  const toggleFilterModal = () => {
    ...
    if (!filterModal) {
      setTempFilterDiseaseName(filterDiseaseName);
    }
  };

  const handleApplyFilters = () => {
    setFilterDiseaseName(tempFilterDiseaseName);
    setCurrentPage(1);
    setFilterModal(false);
  };

  const handleClearFilters = () => {
    setTempFilterDiseaseName("");
  };
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Drugs" breadcrumbItem="Disease List" />
                        ...
                        >
                          ...
                          <tbody>
                            {diseases?.map((disease, index) => (
                              <tr key={disease?.disease_id ?? index}>
                                <td>
                                  {(currentPage - 1) * pageSize + index + 1}
                                </td>
                                <td>{disease?.disease_name ?? "N/A"}</td>
                                ...
                                <td>
                                  {disease?.disease_image ? (
                                    <img
                                      ...
                                      alt={disease?.disease_name ?? "Disease"}
                                      style={{
                                        width: "50px",
                                        height: "50px",
                                        objectFit: "cover",
      ...
      <Modal isOpen={filterModal} toggle={toggleFilterModal} centered size="md">
        ...
        <ModalBody>
          <Form>
            <FormGroup>
              <Label for="diseaseName">Search Disease Name</Label>
              <Input
                type="text"
                id="diseaseName"
                placeholder="Enter disease name"
                value={tempFilterDiseaseName}
                onChange={(e) => setTempFilterDiseaseName(e.target.value)}
              />
            </FormGroup>
          </Form>
        </ModalBody>

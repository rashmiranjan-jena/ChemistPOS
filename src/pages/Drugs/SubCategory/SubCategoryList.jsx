...
const SubCategoryList = () => {
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      try {
        ...
        Swal.fire({
          title: "Error!",
          text: error?.message ?? "Failed to fetch subcategories.",
          icon: "error",
  ...
  const handleEdit = (id) => {
    navigate(`/sub-category-form`, { state: { id } });
  };
    ...
    if (firstConfirmation?.isConfirmed) {
      ...
      if (secondConfirmation?.isConfirmed) {
        try {
          ...
          Swal.fire({
            title: "Error!",
            text: error?.message ?? "Failed to delete the subcategory.",
            icon: "error",
            confirmButtonText: "OK",
          });
        }
  ...
  const handleExcelUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await uploadSubCategoriesExcel(formData);
        Swal.fire({
  ...
  const handleExcelDownload = async () => {
    setDownloading(true);
    try {
      await downloadSubCategoriesExcel();
      Swal.fire({
        title: "Success!",
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Drugs" breadcrumbItem="Sub Category List" />
                      ...
                      <tbody>
                        {loading ? (
                          ...
                          subCategories?.map((subcategory, index) => (
                            <tr key={subcategory?.sub_category_id ?? index}>
                              <td>{index + 1}</td>
                              <td>{subcategory?.sub_category_id ?? "N/A"}</td>
                              <td>{subcategory?.sub_category_name ?? "N/A"}</td>
                              <td>
                                {subcategory?.group_category_name ?? "N/A"}
                              </td>
                              ...
                              <td>
                                {subcategory?.sub_category_description ?? "N/A"}
                              </td>
                              <td>
                                <div className="d-flex gap-2 justify-content-center">
                                  <FaEdit
                                    ...
                                    onClick={() =>
                                      handleEdit(subcategory?.sub_category_id)
                                    }
                                    title="Edit"
                                  />
                                  <FaTrash

...
import {
  getDrugTypes,
  deleteDrugType,
  uploadDrugTypesExcel,
  downloadDrugTypesExcel,
...
const DrugTypeList = () => {
  ...
  const handleEdit = (id) => {
    navigate(`/drug-type-form`, { state: { id } });
  };

  const handleDelete = async (id) => {
    const firstConfirmation = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to delete this drug type.",
      icon: "warning",
    ...
    if (firstConfirmation?.isConfirmed) {
      ...
      if (secondConfirmation?.isConfirmed) {
        try {
          await deleteDrugType(id);
          setDrugTypes(drugTypes.filter((type) => type?.drug_type_id !== id));
          Swal.fire({
            title: "Deleted!",
            text: "The drug type has been successfully deleted.",
            icon: "success",
            confirmButtonText: "OK",
          });
        } catch (error) {
          ...
          Swal.fire({
            title: "Error!",
            text: error?.message ?? "Failed to delete the drug type.",
            icon: "error",
            confirmButtonText: "OK",
          });
        }
  ...
  const handleExcelUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await uploadDrugTypesExcel(formData);
        Swal.fire({
        ...
        const updatedData = await getDrugTypes();
        setDrugTypes(updatedData ?? []);
      } catch (error) {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Drugs" breadcrumbItem="Drug Type List" />
                ...
                <CardBody className="p-4">
                  <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap">
                    ...
                    <div className="d-flex flex-wrap gap-2">
                      <Button
                        color="primary"
                        onClick={() => navigate("/drug-type-form")}
                        style={{
                          height: "35px",
                          padding: "3px 10px",
                          borderRadius: "10px",
                      ...
                      <tbody>
                        {loading ? (
                          ...
                          drugTypes?.map((type, index) => (
                            <tr key={type?.drug_type_id ?? index}>
                              <td>{index + 1}</td>
                              <td>{type?.drug_type_name ?? "N/A"}</td>
                              <td>
                                {type?.drug_type_image ? (
                                  <img
                                    src={`${import.meta.env.VITE_API_BASE_URL}${
                                      type.drug_type_image
                                    }`}
                                    alt={type?.drug_type_name ?? "Drug Type"}
                                    style={{
                                      width: "50px",
                                      height: "50px",
                                      objectFit: "cover",
                                    ...
                                    onClick={() =>
                                      handleImagePreview(type?.drug_type_image)
                                    }
                              ...
                              <td>{type?.drug_type_description ?? "N/A"}</td>
                              <td>
                                <div className="d-flex gap-2 justify-content-center">
                                  ...
                                  <FaTrash
                                    ...
                                    onClick={() =>
                                      handleDelete(type?.drug_type_id)
                                    }
                                    title="Delete"
                                  />
                                </div>

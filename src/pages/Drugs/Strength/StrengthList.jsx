...
import {
  getStrengths,
  deleteStrength,
  uploadStrengthsExcel,
  downloadStrengthsExcel,
...
const StrengthList = () => {
  ...
  const [filterStrengthName, setFilterStrengthName] = useState("");
  const [tempFilterStrengthName, setTempFilterStrengthName] = useState("");
  ...
  useEffect(() => {
    const fetchStrengths = async () => {
      try {
        ...
        Swal.fire({
          title: "Error!",
          text: error?.message ?? "Failed to fetch strengths.",
          icon: "error",
    ...
    fetchStrengths();
  }, [currentPage, filterStrengthName]);
  ...
  const handleEdit = (id) => {
    navigate(`/strength-form`, { state: { id } });
  };

  const handleDelete = async (id) => {
    const firstConfirmation = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to delete this strength.",
      icon: "warning",
    ...
    if (firstConfirmation?.isConfirmed) {
      ...
      if (secondConfirmation?.isConfirmed) {
        try {
          ...
          if (strengths.length === 1 && currentPage > 1) {
          ...
          Swal.fire({
            title: "Deleted!",
            text: "The strength has been successfully deleted.",
            icon: "success",
            confirmButtonText: "OK",
          });
        } catch (error) {
          ...
          Swal.fire({
            title: "Error!",
            text: error?.message ?? "Failed to delete the strength.",
            icon: "error",
            confirmButtonText: "OK",
          });
        }
  ...
  const handleExcelUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await uploadStrengthsExcel(formData);
        Swal.fire({
        ...
        const updatedData = await getStrengths(
          currentPage,
          pageSize,
          filterStrengthName
        );
        setStrengths(updatedData?.data ?? []);
  ...
  const toggleFilterModal = () => {
    ...
    if (!filterModal) {
      setTempFilterStrengthName(filterStrengthName);
    }
  };

  const handleApplyFilters = () => {
    setFilterStrengthName(tempFilterStrengthName);
    setCurrentPage(1);
    setFilterModal(false);
  };

  const handleClearFilters = () => {
    setTempFilterStrengthName("");
  };
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Drugs" breadcrumbItem="Strength List" />
                ...
                <CardBody className="p-4">
                  <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap">
                    ...
                    <div className="d-flex flex-wrap gap-2">
                      <Button
                        color="primary"
                        onClick={() => navigate("/strength-form")}
                        style={{
                          height: "35px",
                        ...
                        >
                          ...
                          <tbody>
                            {strengths?.map((strength, index) => (
                              <tr key={strength?.strength_id ?? index}>
                                <td>
                                  {(currentPage - 1) * pageSize + index + 1}
                                </td>
                                <td>{strength?.strength_name ?? "N/A"}</td>
                                <td>
                                ...
                                <td>
                                  {strength?.strength_description ?? "N/A"}
                                </td>
                                <td>
                                  <div className="d-flex gap-2 justify-content-center">
                                    <FaEdit
                                      ...
                                      onClick={() =>
                                        handleEdit(strength?.strength_id)
                                      }
                                      title="Edit"
                                    />
                                    <FaTrash
                                      ...
                                      onClick={() =>
                                        handleDelete(strength?.strength_id)
                                      }
                                      title="Delete"
                                    />
                                  </div>
      ...
      <Modal isOpen={filterModal} toggle={toggleFilterModal} centered size="md">
        ...
        <ModalBody>
          <Form>
            <FormGroup>
              <Label for="strengthName">Search Strength Name</Label>
              <Input
                type="text"
                id="strengthName"
                placeholder="Enter strength name"
                value={tempFilterStrengthName}
                onChange={(e) => setTempFilterStrengthName(e.target.value)}
              />
            </FormGroup>
          </Form>
        </ModalBody>

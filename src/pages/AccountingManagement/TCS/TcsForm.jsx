...
const TcsForm = () => {
  ...
  // Define validation schema inside component to use isEditMode
  const validationSchema = Yup.object().shape({
    ...
    tcsRate: Yup.number()
      .required("TCS Rate is required")
      .min(0, "TCS Rate cannot be negative")
      .max(100, "TCS Rate cannot exceed 100%"),
  ...
  useEffect(() => {
    if (location?.state?.id) {
      setIsEditMode(true);
      setTcsId(location.state.id);
      fetchTcs(location.state.id);
    }
  }, [location?.state]);

  const fetchTcs = async (id) => {
    try {
      const data = await getTcsById(id);
      formik.setValues({
        party: data?.party || "",
        panNumber: data?.pan || "",
        ...
        tcsRate: parseFloat(data?.tcs_rate) || "",
        paymentDate:
      ...
      Swal.fire({
        title: "Error!",
        text: error?.response?.data?.message || "Failed to fetch TCS data.",
        icon: "error",
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        formData.append("tcs_rate", values.tcsRate);
        formData.append(
          "tcs_deducted",
          ((values.amount * values.tcsRate) / 100).toFixed(2)
        );
        ...
        let response;
        if (isEditMode) {
          response = await updateTcs(tcsId, formData);
          Swal.fire({
            title: "TCS Updated!",
            text: "The TCS details have been successfully updated.",
            icon: "success",
            confirmButtonText: "OK",
          });
        } else {
          response = await postTcs(formData);
          Swal.fire({
            title: "TCS Registered!",
            text: "The TCS details have been successfully registered.",
            icon: "success",
            confirmButtonText: "OK",
          });
        }
  ...
  const calculateTcsDeducted = (amount, tcsRate) => {
    if (amount && tcsRate) {
      return ((parseFloat(amount) * parseFloat(tcsRate)) / 100).toFixed(2);
    }
    return "0.00";
  };
  ...
  document.title = "TCS Registration";
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            ...
            breadcrumbItem={isEditMode ? "Edit TCS Details" : "Add TCS Details"}
          />
              ...
              >
                <CardBody className="p-4">
                  <div className="d-flex justify-content-between align-items-center mb-4">
                    ...
                    >
                      {isEditMode ? "Edit TCS Details" : "Add TCS Details"}
                    </h4>
                    <Button
                      color="secondary"
                      onClick={() => navigate(-1)}
                      ...
                      <Col md="6">
                        <FormGroup>
                          ...
                          <Input
                            ...
                            onBlur={formik.handleBlur}
                            className={`form-control ${
                              formik.touched.invoiceFile &&
                              formik.errors.invoiceFile
                                ? "is-invalid"
                                : ""
                            }`}
                            style={{
                      ...
                      <Col md="6">
                        <FormGroup>
                          <Label
                            ...
                            style={{ fontSize: "14px" }}
                          >
                            TCS Rate (%) <span className="text-danger">*</span>
                          </Label>
                          <Input
                            type="number"
                            name="tcsRate"
                            ...
                            value={formik.values.tcsRate}
                            onChange={formik.handleChange}
                            onBlur={formik.handleBlur}
                            className={`form-control ${
                              formik.touched.tcsRate && formik.errors.tcsRate
                                ? "is-invalid"
                                : ""
                            }`}
                            style={{
                      ...
                      <Col md="6">
                        <FormGroup>
                          ...
                          <Input
                            ...
                            name="tcsDeducted"
                            id="tcsDeducted"
                            value={calculateTcsDeducted(
                              formik.values.amount,
                              formik.values.tcsRate
                            )}
                            readOnly
                            className="form-control"
                            style={{

...
import {
  ...
  uploadTdsExcel,
  downloadTdsExcel,
} from "../../../ApiService/AccountingManagement/Tds";
...
const TdsList = () => {
  ...
  // Fetch TDS records on component mount
  useEffect(() => {
    fetchTdsRecords();
  }, []);
  ...
  const fetchTdsRecords = async () => {
    setLoading(true);
    try {
      const response = await getTdsList();
      console.log("TDS Records:", response); // Debug log
      // Ensure numeric fields are converted
      const formattedRecords = response.map((record) => ({
        ...record,
        amount: parseFloat(record.amount) || 0,
        tds_rate: parseFloat(record.tds_rate) || 0,
        tds_deducted: parseFloat(record.tds_deducted) || 0,
      }));
      setTdsRecords(formattedRecords || []);
      setFilteredRecords(formattedRecords || []);
    } catch (error) {
      Swal.fire({
        title: "Error!",
        text: error?.response?.data?.message || "Failed to fetch TDS records.",
        icon: "error",
  ...
  useEffect(() => {
    ...
    const filtered = tdsRecords.filter(
      (record) =>
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: "Error!",
          text: error?.response?.data?.message || "Failed to delete TDS record.",
          icon: "error",
          confirmButtonText: "OK",
        });
      }
  ...
  const handleExcelDownload = async () => {
    setDownloading(true);
    try {
      await downloadTdsExcel(searchQuery);
  ...
  document.title = "TDS List";

  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="TDS Management" breadcrumbItem="TDS List" />
                        ...
                        <tbody>
                          {filteredRecords.map((record, index) => (
                            <tr key={record?.id}>
                              ...
                              <td>{formatNumber(record?.tds_rate)}%</td>
                              <td>₹{formatNumber(record?.tds_deducted)}</td>
